...
interface AnalyticsSummary {
  ...
  posts: (Post & { account: { pageName: string; platform: string } })[];
  postCount: number;
}
...
export default function AnalyticsPage() {
  ...
  // Load accounts for the property dropdown
  useEffect(() => {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          {selectedAccount && (
            <p className="text-sm text-gray-500 mt-0.5">{selectedAccount.pageName}</p>
          )}
        </div>
      ...
      {data && data.postCount === 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center">
          <p className="text-sm font-medium text-amber-800">No analytics data yet</p>
          <p className="text-sm text-amber-600 mt-1">
            Click <strong>Refresh</strong> to pull the latest metrics from Instagram for{" "}
            {selectedAccount ? selectedAccount.pageName : "all properties"}.
          </p>
        </div>
      )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {["Property", "Platform", "Caption", "Published", "Impressions", "Reach", "Likes", "Comments"].map((h) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {(data?.posts ?? []).map((post) => (
                <tr key={post.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500">{post.platform}</td>
